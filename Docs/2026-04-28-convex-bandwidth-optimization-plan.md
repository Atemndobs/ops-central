...
| 3.b — Audit and narrow individual subscription mount points | ✅ Shipped | 2026-05-01 | Slice 1 (PR #48): `countByProperty` + `countByCleaner` thin counters; switched job-detail "Related Activity" panel from unbounded enriched `getAll`/`getForCleaner` (used only for `.length`) to count-only reads. Eliminates a full-table `cleaningJobs.collect()` in `getForCleaner` on every job-detail mount. Slice 2 (PR #52): `getAssignable` (active-statuses-only via `by_status` index, the four assignable states) and `getInDateRange` (windowed via `by_scheduled` index); switched team page from `getAll({ limit: 1000 })` to `getAssignable` and schedule-client from `getAll({ limit: 1000 })` to `getInDateRange` keyed on the visible window. Slice 3: `notEndedBefore` arg added to `getAll` (server-side filter mirroring the jobs-page `hidePastJobs` toggle), and dashboard-client switched from `getAll({ limit: 500 })` to `getInDateRange` keyed on a past-7d → future-30d window. |
...
| 5.b — Switch `getMyAssigned` read to `userJobAssignments` | ✅ Shipped | 2026-04-30 | PR #43 — read uses index lookup + targeted `ctx.db.get` per job. Verified backfill on whimsical (52 rows, idempotent). On `usable-anaconda-394`, **getMyAssigned will return empty until backfill runs** at consolidation; deployment is paused so no live impact. |
| 6 — `getMyAccessibleProperties` reuses `userJobAssignments` index | ✅ Shipped | 2026-05-01 | PR #44 — same fix as Wave 5.b but for the property picker on cleaner UI. Eliminates the second `cleaningJobs.collect()` on a hot path. |
