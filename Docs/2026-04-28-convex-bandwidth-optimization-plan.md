...
| 3.b — Audit and narrow individual subscription mount points | 🟡 In progress | 2026-05-01 | First slice: `countByProperty` + `countByCleaner` thin counters; switched job-detail "Related Activity" panel from unbounded enriched `getAll`/`getForCleaner` (used only for `.length`) to count-only reads. Eliminates a full-table `cleaningJobs.collect()` in `getForCleaner` on every job-detail mount. Remaining: schedule-client (limit 1000), team page (limit 1000), jobs-page-client, dashboard-client. |
