...
| 3.b — Audit and narrow individual subscription mount points | 🟡 In progress | 2026-05-01 | Slice 1 (PR #48): `countByProperty` + `countByCleaner` thin counters; switched job-detail "Related Activity" panel from unbounded enriched `getAll`/`getForCleaner` (used only for `.length`) to count-only reads. Eliminates a full-table `cleaningJobs.collect()` in `getForCleaner` on every job-detail mount. Slice 2: `getAssignable` (active-statuses-only via `by_status` index, the four assignable states) and `getInDateRange` (windowed via `by_scheduled` index); switched team page from `getAll({ limit: 1000 })` to `getAssignable` and schedule-client from `getAll({ limit: 1000 })` to `getInDateRange` keyed on the visible window. Remaining: jobs-page-client (paginate), dashboard-client (per-status counters). |
